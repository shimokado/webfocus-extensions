...
⚠️ **重要**: WebFOCUSから渡されるデータの構造は可変的です。renderCallback の最初に必ず正規化してください。

**よくある誤り**：`depth === 1` だからといって `data = [data]` にラップしてはいけません。depth=1 の `data` は既にアイテム配列です。
...
function renderCallback(renderConfig) {
  ...
  // 以降、normalized.data は常に統一形式で安全にアクセス可能
...
- **詳細な実装例**: `com.shimokado.params` - コンソール出力で正規化前後のデータ構造を視覚的に示します
- **詳細な解説**: `development_guide/02_API_Reference.md` の Section 3.5 を参照
- **実装パターン**: `development_guide/03_Development_Guide.md` の Section 1 を参照
